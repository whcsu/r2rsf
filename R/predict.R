...
##' @export
predict.rangernew.forest <- function(object, data, predict.all = FALSE,
  ...
  if (class(object) != "rangernew.forest") {
  ...
  ## Call Ranger
  result <- rangernewCpp(treetype, dependent.variable.name, data.final, variable.names, mtry,
  ...
  class(result) <- "rangernew.prediction"
  return(result)
}
...
##' @author Marvin N. Wright
##' @export
predict.rangernew <- function(object, data, predict.all = FALSE,
                           num.trees = object$num.trees,
                           type = "response",
                           seed = NULL, num.threads = NULL,
                           verbose = TRUE, ...) {
  ...
  if (is.null(forest)) {
    stop("Error: No saved forest in rangernew object. Please set write.forest to TRUE when calling rangernew.")
  }
