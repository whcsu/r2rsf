...
##' @aliases predictions
##' @export
predictions.rangernew.prediction <- function(x, ...) {
  if (class(x) != "rangernew.prediction") {
    stop("Object ist no rangernew.prediction object.")
  }
...
##' @author Marvin N. Wright
##' @export
predictions.rangernew<- function(x, ...) {
  if (class(x) != "rangernew") {
    stop("Object ist no rangernew object.")
  }
  if (x$treetype == "Classification" | x$treetype == "Regression" | x$treetype == "Probability estimation") {
    if (is.null(x$predictions)) {
      stop("No predictions found.")
