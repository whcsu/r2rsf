...
public:
 ...
 std::vector<std::vector<std::vector<double>>> getPtime() const {
      std::vector<std::vector<std::vector<double>>>result2;
    result2.reserve(num_trees);
    for (Tree* tree : trees) {
      TreeSurvival* temp = (TreeSurvival*) tree;
      result2.push_back(temp->getPtime());
    }
    return result2;
  }
