...
TreeSurvival::TreeSurvival(std::vector<std::vector<size_t>>& child_nodeIDs, std::vector<size_t>& split_varIDs,
    std::vector<double>& split_values, std::vector<std::vector<double>> chf,std::vector<std::vector<double>> ptime, std::vector<double>* unique_timepoints,
    ...
    Tree(child_nodeIDs, split_varIDs, split_values, is_ordered_variable), status_varID(0), unique_timepoints(
        unique_timepoints), response_timepointIDs(response_timepointIDs), chf(chf), ptime(ptime),num_deaths(0), num_samples_at_risk(
        0) {
  this->num_timepoints = unique_timepoints->size();
...
void TreeSurvival::appendToFileInternal(std::ofstream& file) {
 ...
 //added by HongWang
   std::vector<std::vector<double>> ptime_vector;
   
  for (size_t i = 0; i < ptime.size(); ++i) {
    if (!ptime[i].empty()) {
      ptime_vector.push_back(ptime[i]);
    }
  }
   
  saveVector2D(ptime_vector, file);
}

void TreeSurvival::createEmptyNodeInternal() {
  chf.push_back(std::vector<double>());
  //added by Hong Wang
  ptime.push_back(std::vector<double>());
 }
...
void TreeSurvival::computeSurvival(size_t nodeID) {
  ...
  for (size_t i = 0; i < num_timepoints; ++i) {
    if (num_samples_at_risk[i] != 0) {
	 ...
	 // std::cout<<"chf_value="<<chf_value <<" timeid="<<i<<"\n";
    }
...
void TreeSurvival::estimatetime(size_t nodeID) {
 
   // size of a given node: samples within this node	
 ...
 // Order by time for calacuate km survival
  ...
  //get sorted time & status 
   ...
   newtime.reserve(num_samples_node);
   std::vector<double> newstatus;
   newstatus.reserve(num_samples_node);
   
   for (size_t i = 0; i < indices.size(); ++i) {
        newtime.push_back(data->get(sampleIDs[nodeID][indices[i]], dependent_varID));
		newstatus.push_back(data->get(sampleIDs[nodeID][indices[i]], status_varID));
   }
   
  // calculate km alues 
  std::vector<double> km_temp;
  km_temp.reserve(num_samples_node);
  // std::fill(km_temp.begin(), km_temp.end(), 0.0);
    
  double sum_time = 0.0;
  
  double status_double = (double) newstatus[0];
  double num_samp_at_risk = (double) num_samples_node;  
  
  km_temp.push_back(1-status_double/num_samp_at_risk);

  
  for( size_t i=1; i<num_samples_node; i++){
	status_double = (double) newstatus[i]; 
    km_temp.push_back(km_temp.back()*(1-status_double/(num_samp_at_risk-i)));
    sum_time=sum_time+newtime[i]*(km_temp.end()[-2]-km_temp.end()[-1]);
   }
  
  
  
  std::vector<double> ptime_temp;
  //only one time need to be stored
  ptime_temp.reserve(1);
  
  //std::cout<<"sumptime="<<sum_responses_in_node / (double) num_samples_in_node<<" nodeid="<<nodeID<<"\n";
  ptime_temp.push_back(sum_time);
  ptime[nodeID]=ptime_temp;
...
bool TreeSurvival::findBestSplit(size_t nodeID, std::vector<size_t>& possible_split_varIDs) {
    ...
    for (auto& varID : possible_split_varIDs) {
      ...
      if ((*is_ordered_variable)[varID]) {
        if (splitrule == LOGRANK) {
		 ...
		 else if (splitrule == R2) {
          findBestSplitValueR2(nodeID, varID, best_value, best_varID, best_decrease);
        }
      } else {
  ...
  if (best_decrease < 0) {
	...
	estimatetime(nodeID);
	//std::cout<<splitrule<<"Rule"<<nodeID<<"\n";
...
// Following coded added by Hong Wang

void TreeSurvival::computekmweights(size_t nodeID, std::vector<size_t> &indices, std::vector<double> &kmweights) {
  // size of a given node: samples within this node	
  size_t num_samples_node = sampleIDs[nodeID].size();
  ...
  double status_double = (double) newstatus[0];
  double num_samp_at_risk = (double) num_samples_node;  
  km_temp.push_back(1.0);
  
  for( size_t i=0; i<num_samples_node-1; i++){
	status_double = (double) newstatus[i]; 
    km_temp.push_back(km_temp.back()*(1-(1-status_double)/(num_samp_at_risk-i)));
   ...
   if (km_temp[i]>0)  sum=sum+(status_double/km_temp[i]);
   } 

  std::vector<double> kmw_temp;
  kmw_temp.reserve(num_samples_node);
  
  for( size_t i=0; i<num_samples_node; i++){
	status_double = (double) newstatus[i]; 
    if (km_temp[i]>1e-5) 
		kmw_temp.push_back((status_double/km_temp[i])/sum);
	else 
		kmw_temp.push_back(0.0);	
  }
  for( size_t i=0; i<num_samples_node; i++){
    //stored the weights in its sorted order 
	kmweights[i]=kmw_temp[indices[i]]; 
   }
}


double TreeSurvival::compute_weightdotprod(std::vector<double> scalar_w, std::vector<double> scalar_x, std::vector<double> scalar_y){
...
double TreeSurvival::compute_splitr2(std::vector<double>  scalar_w, std::vector<double> scalar_x, std::vector<double> scalar_y,  std::vector<double> scalar_ypre){
   double r2_value=0;
	...
	std::vector<double> scalar_tempy;
	scalar_tempy.reserve(scalar_y.size());
     for (size_t i=0;i<scalar_y.size();i++){
	  scalar_tempy.push_back(scalar_y[i]-wy);
	 }
	 
    // time pre-m theta w 
	std::vector<double> scalar_temppre;
	scalar_temppre.reserve(scalar_ypre.size());
     for (size_t i=0;i<scalar_ypre.size();i++)
	   scalar_temppre.push_back(scalar_ypre[i]-wypre);
   ...
   std::vector<double> ptwsqure;
   ptwsqure.reserve(scalar_ypre.size());
   for (size_t i=0;i<scalar_ypre.size();i++)
	   ptwsqure.push_back((scalar_ypre[i]*slope+intercept-wy)*(scalar_ypre[i]*slope+intercept-wy));
     
   double r2_numerator=std::inner_product(scalar_w.begin(), scalar_w.end(), ptwsqure.begin(), 0.0);
      
   
   //twsqure=(yi-wy)^2;
   std::vector<double> twsqure;
   twsqure.reserve(scalar_y.size());
   for (size_t i=0;i<scalar_y.size();i++)
	   twsqure.push_back((scalar_y[i]-wy)*(scalar_y[i]-wy));
...
void TreeSurvival::findBestSplitValueR2(size_t nodeID, size_t varID, double& best_value, size_t& best_varID,
    double& best_r2) {
  ...
  // Initialize for kmweights
  
  std::vector<double> kmweights;  
  kmweights.reserve(sampleIDs[nodeID].size());
  kmweights.assign(sampleIDs[nodeID].size(), 0.0);
 
 // Get all observations of x, time and status data
  std::vector<double> time;
  time.reserve(num_samples_node);
  std::vector<double> status;
  status.reserve(num_samples_node);
 ...
 // Order by time for calacuate km survival
  ...
  //compute sample weights
   computekmweights(nodeID,indices,kmweights);
  
  
  std::vector<size_t> num_samples_left_child;
  num_samples_left_child.reserve(num_splits);  
  
  num_samples_left_child.assign(num_splits, 0);
  
  for (size_t k = 0; k < num_samples_node; ++k) {
     // Count samples in left node
	 for (size_t i = 0; i < num_splits; ++i) {
        if (x[k] <= possible_split_values[i]) {		
        ++num_samples_left_child[i];		 
      }
    }
  }
  ...
  // Order by x for splitting
   std::vector<size_t> index_x = order(x, false);
   
  
   // Compute r2 values at different split values
  double r2=0;
  for (size_t i = 0; i < num_splits; ++i) {
   
    // Stop if minimal node size reached
    size_t num_samples_right_child = num_samples_node - num_samples_left_child[i];
    if (num_samples_left_child[i] < min_node_size || num_samples_right_child < min_node_size){
      continue;
    }
     // Compute r2 values statistic 
    std::vector<double> curr_x;
    curr_x.reserve(num_samples_node);
	
    curr_x.assign(num_samples_node, 0.0);	

	std::fill(curr_x.begin()+num_samples_left_child[i]+1, curr_x.end(), 1.0);
	
	std::vector<double> curr_y;
	curr_y.reserve(num_samples_node);
	std::vector<double> curr_kwm;
    curr_kwm.reserve(num_samples_node);	
	
	

	
    for (size_t j = 0; j < num_samples_node; ++j){
		curr_y.push_back(time[index_x[j]]);
		curr_kwm.push_back(kmweights[index_x[j]]);
	}
      
	
     r2 =compute_splitr2(curr_kwm,curr_x,curr_y,curr_x);
