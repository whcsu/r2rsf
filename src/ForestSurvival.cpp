...
void ForestSurvival::loadForest(size_t dependent_varID, size_t num_trees,
    ...
    size_t status_varID, std::vector<std::vector<std::vector<double>> >& forest_chf,std::vector<std::vector<std::vector<double>> >& forest_ptime,
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    Tree* tree = new TreeSurvival(forest_child_nodeIDs[i], forest_split_varIDs[i], forest_split_values[i],
        forest_chf[i], forest_ptime[i], &this->unique_timepoints, &response_timepointIDs, &this->is_ordered_variable);
...
void ForestSurvival::initInternal(std::string status_variable_name) {
  ...
  if (!prediction_mode) {
    for (size_t i = 0; i < num_samples; ++i) {
	  ...
	  // if find, return the index, If no elements match, the function returns last.
...
void ForestSurvival::predictInternal() {
  ...
  timepredictions.reserve(num_prediction_samples);
...
// First dim trees, second dim samples, third dim time
  for (size_t i = 0; i < num_prediction_samples; ++i) {
    ...
    if (prediction_type == TERMINALNODES) {
	  ...
	  // added here Mean over trees
      double prediction_sum = 0; 
	  for (size_t tree_idx = 0; tree_idx < num_trees; ++tree_idx) {
        prediction_sum += ((TreeSurvival*) trees[tree_idx])->getTimePrediction(i)[0];
      }
      std::vector<double> temp;
      temp.push_back(prediction_sum / num_trees);
      timepredictions.push_back(temp);
...
//    }
    ...
    readVector2D(ptime_vector, infile);
	
	// Convert chf to vector with empty elements for non-terminal nodes
    std::vector<std::vector<double>> ptime;
    ptime.resize(child_nodeIDs[0].size(), std::vector<double>());
    for (size_t j = 0; j < terminal_nodes.size(); ++j) {
      ptime[terminal_nodes[j]] = ptime_vector[j];
    }
    ...
    // Create tree
    Tree* tree = new TreeSurvival(child_nodeIDs, split_varIDs, split_values, chf, ptime, &unique_timepoints,
