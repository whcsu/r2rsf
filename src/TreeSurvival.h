...
public:
  ...
  const std::vector<std::vector<double> >& getChf() const {
   ...
   const std::vector<std::vector<double> >&  getPtime() const {
    return ptime;
  }
  const std::vector<double>& getPrediction(size_t sampleID) const {
   ...
   const std::vector<double>& getTimePrediction(size_t sampleID) const {
    size_t terminal_nodeID = prediction_terminal_nodeIDs[sampleID];
	//std::cout<<"Tid="<<terminal_nodeID<< " ptime="<<ptime[terminal_nodeID][0]<<"\n";
	
    return (ptime[terminal_nodeID]);
  }
...
private:
  ...
  // added by Hong Wang
  void estimatetime(size_t nodeID);

  void computekmweights(size_t nodeID, std::vector<size_t> & indices, std::vector<double>& kmweights);
  // added by Hong Wang
  double compute_weightdotprod(std::vector<double>  scalar_w, std::vector<double> scalar_x, std::vector<double> scalar_y);
  // added by Hong Wang
  // double compute_splitr2(std::vector<double>  scalar_w, std::vector<double> scalar_x, std::vector<double> scalar_y);
  
  double compute_splitr2(std::vector<double>  scalar_w, std::vector<double> scalar_x, std::vector<double> scalar_y,std::vector<double> scalar_ypre);
  ...
  void findBestSplitValueR2(size_t nodeID, size_t varID, double& best_value, size_t& best_varID, double& best_r2);
  ...
  std::vector<std::vector<double>> ptime;
